...
console.log( text.replace(/(Manuel Marques de Sousa)/gi , "Maxsuel Storch Barboza Rodrigues"))
...
console.log( text.replace(/(brasileiro)/gi , "capixaba"))
...
console.log( text.replace(/(O Centauro de Luvas)/gi , function(regex, parte1 ){
    return parte1.toUpperCase();
} ))
...
// isso -> var regexDate = /(13) de (julho) de (1804) /
// vira isso
...
console.log( '\nReplace de datas:' );
// ?
var replaceName = function replaceName( regex, parte1, parte2, parte3){
    return parte1 + "/" + getMonthNumber( parte2 ) + "/" + parte3;
}

console.log( text.replace( regexDate , replaceName ))
}) ();